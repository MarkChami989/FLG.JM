    ...
    <>
      ...
      <main>
        <div className="reserve-card">
          ...
          <div className="form-header">
            <span className="form-icon"><Icon paths={ICONS.ppIcon} width="40" height="40" /></span>
            <div className="form-tag">Tabletop Games</div>
            <div className="form-title">Reserve Ping Pong Table</div>
          </div>
          ...
          <div className="block">
            ...
            <div className="table-grid">
              {TABLES.map((name) => (
                <div key={name} className={`table-opt${selTable === name ? ' selected' : ''}`} onClick={() => selectTable(name)}>
                  <div className="table-check">✓</div>
                  <div className="pp-art"><div className="pp-ball"></div></div>
                  <div className="table-name">{name.toUpperCase()}</div>
                  <div className="table-cap">2 Players</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className={`success-overlay${showSuccess ? ' show' : ''}`}>
        <div className="success-box">
          <div className="success-icon"><Icon paths={ICONS.ppIcon} width="52" height="52" /></div>
          <div className="success-title">Table Reserved!</div>
          <div className="success-sub">{successMsg}</div>
          <button className="btn-close" onClick={() => setShowSuccess(false)}>Let's Play!</button>
        </div>
      </div>
    </>
  )
